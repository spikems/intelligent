...
class Learner():
    ...
    def init_vectorizer(self, type='tfidf', ngram_range=(1, 1), max_df=0.9):
        ...
        """
        if type == 'tfidf':
            self.vectorizer = TfidfVectorizer(sublinear_tf=True, max_df=max_df, ngram_range=ngram_range,
                                              vocabulary=self.vocabulary, token_pattern=r'[^ ]+')
        # self.vectorizer = TfidfVectorizer(analyzer=str.split, sublinear_tf=True, max_df=max_df, ngram_range=ngram_range, vocabulary = self.vocabulary)
            ...
            self.vectorizer = CountVectorizer(binary=True, max_df=max_df, ngram_range=ngram_range,
                                              vocabulary=self.vocabulary, token_pattern=r'[^ ]+')
    ...
    def load_dataset(self, fname, category_names=[], recordid=True, bin_class=-999):
        ...
        if category_names and targetCnt > len(category_names):
            ...
            logger.info("loading dataset with %d documents - %0.3fMB, %d categories.", len(data), data_train_size_mb,
                        len(category_names))
