...
class PreDeal():
    ...
    def __getfeats(self, content='', seltype=0):
        ...
        if seltype == SEL_FTONLY:
            ...
            # 把一个大句子content根据标点符号分成短句sentences
            ...
            for sentence in sentences:
                ...
                for _w in items:
                    ...
                    word = winfo[0].strip()
                    if word:
                        # 有FT_MARKER标记的就是特证词，在上个函数__maketag中打上的
                        if winfo[1].find(FT_MARKER) > 0:
                            ...
                            # 如果特证词类型在下边列表里，做成一个字典ft_pos,key为标签类型，value为词性
    ...
    def run(self, sent, wordmap, target, seltype=1):
        result = ''
        iRange = 100
        sent = ExtractShortText().extract_short_text(target, sent, iRange)
        ...
        if result.find(FT_OPINION) == -1 or result.find(FT_TARGET) == -1:
            result = ''
        return result, sent.encode('utf-8')
