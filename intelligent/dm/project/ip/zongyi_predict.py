...
class PredictZongYi(object):
    ...
    def rule_filter(self, name_position_data, text):
        ...
        try:
            ...
            error = "zongyi_rule_filter error. traceback: %s" % traceback.format_exc()
    ...
    def process_data(ip_word, position_data, content, ip_type, tag=None):
        ...
        if word_num < 2:
            for word in zongyi_full_text_related_words:
                if word in content:
                    first_process_data = 'word%d,' % word_num + first_process_data  # 当出现一次以上的词，添加一个权重词word1
                    word_num += 1
                if word_num == 2:
                    break
    ...
    def predict_result(self, models, ip_words, ip_type, text, name_position_data):
        ...
        try:
            ...
            error = "zongyi_predict.run error. traceback: %s" % traceback.format_exc()
            self.err_logger.error(error)
            return None
