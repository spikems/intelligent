...
def load_template_feature(result_with_flag, posList, dBrand, lFeatureTemplate, sBrand):
    '''
        ...
        if sBrand == sWord:
            for sItem in lFeatureTemplate:
                ...
                if len(lFlagNum) == 2:
                    ...
                    iMax, iSmall = (int(lFlagNum[1]), int(lFlagNum[2])) if int(lFlagNum[1]) > int(lFlagNum[2]) else (
                    int(lFlagNum[2]), int(lFlagNum[1]))
                    if iMax * iSmall > 0:
                        if iMax > 0:
                        ...
                        sFeature = 'local_%s_brand_%s' % (
                        "_".join(lBeforeQueue[sPosFlag][iSmall:]), "_".join(lAfterQueue[sPosFlag][:iMax]))
...
def cut_text_plus(content, posFlag, lFeatureTemplate, dBrand, sBrand):
    '''
     cut text and add context feature

    '''

    bIsBrand, result, result_with_flag, posList, sBrand = cut_input_plus(content, sBrand)

    template_feature = ''
    if bIsBrand and len(lFeatureTemplate) and len(dBrand):
        template_feature = load_template_feature(result_with_flag, posList, dBrand, lFeatureTemplate, sBrand)
