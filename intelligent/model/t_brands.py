# encoding:utf8
# !/usr/bin/env python
from peewee import (Model, IntegerField, CharField, DateTimeField, TextField, PrimaryKeyField, ForeignKeyField,
                    DateField, FloatField, BigIntegerField)
...
class TBrands(ModelBase):
    ...
    def addBrands(self, sBrand='', sDeterminer='', sFdeter='', sSource=1, sThre=0.5, sNoise=1, sIndustry=-1):
        ...
        sSql = "insert into %s(brands_name, determiner, fdeterminer, source, threshold, noise_brand, industry) VALUES ('%s', '%s', '%s', %s, %s, %s, %s)" % (
            self.table, sBrand, sDeterminer, sFdeter, iSource, fThre, iNoise, iIndustry)
    ...
    def updateBrands(self, sBrand='', sDeterminer='', sFdeter='', sIndustry=''):
        ...
        sSql = "update %s set determiner = '%s', fdeterminer = '%s' where brands_name = '%s' and industry = %s" % (
            self.table, sDeterminer, sFdeter, sBrand, sIndustry)
        bRet = super(TBrands, self).operate(sSql)
        return bRet
