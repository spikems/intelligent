...
class CarDiscernNature(object):   # 这里可以构建一个父类，不同的方法，子类进行重构完成
    def __init__(self):
        self.rule_data_path = project_path + '/conf/car_filter_word.xlsx'
    ...
    def get_duplicate_list(list_data):  # 获取重复的列表数据
        content_data_list = []
        for x in list_data:
            if list_data[x][1] == u'微博':  # 汽车微博去重的数据特殊处理
                content_data_list.append(list_data[x][2])
            else:
                content_data_list.append(list_data[x][2] + u',' + list_data[x][3][:40])
        cut_data_list = []
        ...
        for d_l in duplicate_list:
            temp_dict = {}
            for i in d_l:
                origin_classification = list_data[i][5]   # 不同类别的重复对象只留其中一个
                temp_dict[origin_classification] = temp_dict.get(origin_classification, []) + [i]
            for source in temp_dict:
                if len(temp_dict[source]) > 1:
                    temp_dict[source].pop()  # 保留一个重复的为一个自然声量
    ...
    def rule_filter(self, list_data, duplicate_list):
        ...
        for data_id in list_data:
            ...
            if list_data[data_id][1] == u'微博':   # 汽车只过滤微博的问题
                # 规则2：title中含有过滤词
                title_judge = False
                for word in self.filter_words['title_rule_words']:
                    if word == u'#':
                        if word in list_data[data_id][2][:2]:
                            if u'【' in list_data[data_id][2]:
                ...
                for word in self.filter_words['author_rule_words']:
                    if word in list_data[data_id][4]:
                        ...
                        author_judge = True
                        break
                if not author_judge and list_data[data_id][-1] in list_data[data_id][4]:
                    one_result = self.process_result(list_data[data_id], u'营销', u'品牌名(作者)')
                    final_str_data.append(one_result)
                    author_judge = True
                if author_judge:
                    continue
    ...
    def run(self, raw_data_path, save_path):
        ...
        excel_title = [u'序号', u'分类', u'主题', u'摘要', u'作者', u'监控对象', u'(汽车)结果', u'判断依据']
