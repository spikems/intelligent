...
class MzDiscernNature(object):
    def __init__(self):
        self.rule_data_path = project_path + '/conf/meizhuang_filter_word.xlsx'
        ...
        self.mz_pattern = re.compile(ur'^[\w \t]*【')  # 出现数字字母【的情况
    ...
    def rule_filter(self, list_data, duplicate_list):
        ...
        for data_id in list_data:
            ...
            if list_data[data_id][1] == u'微博':
                # 使用正则表达式匹配的新逻辑
                alnum_result_1 = self.alnum_pattern.match(list_data[data_id][2])
                mz_pattern_1 = self.mz_pattern.match(list_data[data_id][2])
                alnum_result_2 = self.alnum_pattern.match(list_data[data_id][3])
                mz_pattern_2 = self.mz_pattern.match(list_data[data_id][3])
                if alnum_result_1 or mz_pattern_1 or alnum_result_2 or mz_pattern_2:
                    one_result = self.process_result(list_data[data_id], u'营销', u'字母//(开头)')
                    final_str_data.append(one_result)
                    start_judge = True
    ...
    def run(self, raw_data_path, save_path):
        ...
        excel_title = [u'序号', u'分类', u'主题', u'摘要', u'作者', u'监控对象', u'(美妆)结果', u'判断依据']
